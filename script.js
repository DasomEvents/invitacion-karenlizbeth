...
(function () {
  ...
  // Compatibilidad entre navegadores: cada función de abajo (countdown,
  // animaciones al aparecer, música, botón de WhatsApp, etc.) se ejecuta
  // dentro de este envoltorio "safeRun" en vez de llamarse directo. La
  // razón: sin esto, si UNA sola función lanzara un error en algún
  // navegador o dispositivo en particular (uno más viejo, el navegador
  // interno de alguna app, etc.), ese error detendría la ejecución de
  // TODO el resto del script — incluyendo partes tan importantes como el
  // botón de confirmar asistencia por WhatsApp o la aparición del propio
  // contenido de la página. Con "safeRun", si algo falla, solo esa
  // función en particular se salta (y queda registrado en la consola del
  // navegador para poder diagnosticarlo), pero todo lo demás sigue
  ...
  function safeRun(label, fn) {
    try {
      ...
      if (window.console && console.error) {
        console.error("Invitación — error en \"" + label + "\":", err);
      }
    }
  }
   ...
   * ------------------------------------------------------------------ */
  safeRun("resetScrollPosition", function resetScrollPosition() {
   ...
   * 1) Cuenta regresiva
   * ------------------------------------------------------------------ */
  safeRun("initCountdown", function initCountdown() {
    ...
    // Nota de compatibilidad: se arma el "00" a mano en vez de usar
    // String.prototype.padStart, que no existe en algunos navegadores
    // antiguos (por ejemplo el navegador integrado de algunas apps).
    function pad(n) {
   ...
   * ------------------------------------------------------------------ */
  safeRun("initReveal", function initReveal() {
  ...
  // Red de seguridad adicional: si por cualquier motivo "initReveal" no
  // pudo marcar algún elemento como visible (incluido el caso, ya poco
  // probable con "safeRun", de que fallara antes de llegar a hacerlo),
  // esto asegura que nadie se quede viendo una página en blanco: pasados
  // 2.5s, cualquier elemento que siga oculto se muestra igualmente, sin
  // animación.
  safeRun("revealFallback", function revealFallback() {
    window.setTimeout(function () {
      var stuck = document.querySelectorAll("[data-reveal]:not(.is-visible)");
      stuck.forEach(function (el) { el.classList.add("is-visible"); });
    }, 2500);
  });
   ...
   * ------------------------------------------------------------------ */
  safeRun("initScrollProgress", function initScrollProgress() {
  ...
  safeRun("initParticles(portada)", function () { initParticles("particles"); });
  safeRun("initParticles(cierre)", function () { initParticles("particles2"); });
   ...
   * ------------------------------------------------------------------ */
  safeRun("initGlobalSparkles", function initGlobalSparkles() {
   ...
   * ------------------------------------------------------------------ */
  safeRun("initMusic", function initMusic() {
   ...
   * ------------------------------------------------------------------ */
  safeRun("initRSVP", function initRSVP() {
